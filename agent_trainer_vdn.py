...
from networks import SharedDRQN, StaticMapEncoder, VDNMixer
...
class AgentTrainer:
    def __init__(self, obs_channels, num_actions, map_channels, num_agents, device='cuda', lr=1e-4):
        ...
        self.eval_mixer = VDNMixer().to(self.device)
    ...
    def train_step(self, batch, gamma=0.99):
        ...
        with torch.amp.autocast('cuda', enabled=self.use_scaler):
            ...
            for t in range(burn_in, T):
                ...
                dones_t = dones[:, t] # (B, N)
                q_tot_eval = self.eval_mixer(chosen_q_eval, dones_t)
                ...
                with torch.no_grad():
                    ...
                    q_tot_target = self.target_mixer(chosen_q_target, dones_t)
